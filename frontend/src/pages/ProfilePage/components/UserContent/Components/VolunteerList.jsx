...
const VolunteerList = () => {
  ...
  const {
    ...
    error,
    loading,
  } = useListApi("volunteer", pageNum, pageSize);
      ...
      <Modal isOpen={isOpen} title={"출석 확인"}>
        <div style={{ minWidth: "280px", maxWidth: "320px" }}>
          ...
          <Input
            ...
            type="text"
            placeholder="출석 인증 코드를 입력해주세요."
          />
          <div style={{ display: "flex", flexDirection: "row" }}>
            <button
              className="modal_btn positive"
              type="submit"
              onClick={(e) => {
                e.preventDefault();
              ...
              <div className="item_sec w-1">
                <p className="weak">
                  ...
                  Number(year + month + day) <
                    Number(todayYear + todayMonth + todayDay)
                    ? "봉사 불참"
                    : ""}
                  {data.status === "REGISTER" &&
                  Number(year + month + day) >=
              ...
              <div className="item_sec w-2">
                ...
                {data.status === "REGISTER" &&
                  Number(year + month + day) >
                    ...
                    <button
                      className="btn"
                      onClick={() => volunteerCancel(data.volunteerId)}
                    >
                      취소하기
                    </button>
                  )}
                ...
                {data.status === "WRITTEN" ? (
                  <button
                    className="btn"
                    onClick={() => {
                      navigate("/review/" + data.articleId, {
                        state: {
                          volunteerId: data.volunteerId,
                          projectId: data.projectId,
                        },
